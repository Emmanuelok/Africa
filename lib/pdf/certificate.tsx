...
export function CertificatePDF({ data, qrCode }: { data: CertificateData; qrCode?: string }) {
  ...
  const accent = brand?.primaryColor && /^#[0-9a-fA-F]{6}$/.test(brand.primaryColor)
    ? brand.primaryColor
    : colors.terracotta;
  const isWhiteLabel = !!brand?.name;
    ...
    >
      <Page size="A4" style={styles.page}>
        ...
        <View style={[styles.header, { borderBottomColor: accent }]}>
          <View>
            {isWhiteLabel && brand?.logoUrl ? (
              <Image src={brand.logoUrl} style={{ width: 96, height: 32 }} />
            ) : (
              <Text style={styles.logo}>
                {isWhiteLabel ? brand!.name : "Sokoni"}
                {!isWhiteLabel && <Text style={[styles.logoDot, { color: accent }]}>.</Text>}
              </Text>
            )}
            <Text style={styles.title}>AfCFTA Certificate of Origin</Text>
          </View>
          <View style={styles.refBox}>
            <Text style={[styles.ref, { color: accent }]}>{data.reference}</Text>
        ...
        <View style={styles.footer} fixed>
          <Text>
            {isWhiteLabel ? `${brand!.name} · ` : "Generated by Sokoni · sokoni.africa · "}
            Ref {data.reference}
